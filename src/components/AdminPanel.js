...
const AdminPanel = () => {
  ...
  useEffect(() => {
    // Don't check auth on mount - always require fresh login
  ...
  const handleFeatureSaveManual = async (mode, index) => {
    try {
      ...
      alert('Error saving feature: ' + (error.response?.data?.error || error.message));
      setSaving(false);
    }
  };
  ...
  const syncAllChanges = async () => {
    try {
      ...
      console.log('Fresh content from server:', freshContent);
      setTempContent(freshContent);
  ...
  // Debug: Log content differences
  console.log('Current content features (men):', content.men?.features?.length);
  console.log('Temp content features (men):', tempContent.men?.features?.length);
  console.log('Current content features (women):', content.women?.features?.length);
  console.log('Temp content features (women):', tempContent.women?.features?.length);
        ...
        <div className="admin-status">
          {connectionError && (
            <div className="connection-error">
              ⚠️ Server Disconnected - Changes will not be saved
            </div>
          )}
